...
@php
    ...
    $typeBadgeColors = [
        'press_release' => 'bg-info-light text-info',
        'announcement' => 'bg-warning-light text-warning',
        'news' => 'bg-success-light text-success',
    ];
    $badgeClass = $typeBadgeColors[$broadcast->type] ?? 'bg-gray-100 text-muted';
@endphp

<article class="group bg-white rounded-xl border border-border hover:border-primary hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col">
    {{-- Featured image --}}
    ...
    <div class="p-5 flex flex-col flex-1">
        ...
        <div class="flex items-center gap-3 mb-3">
            @if($broadcast->type)
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badgeClass }}">
                    {{ __('home.broadcasts.type.' . $broadcast->type) }}
                </span>
            @endif
        ...
        {{-- Title --}}
        <h3 class="font-semibold text-text group-hover:text-primary transition-colors line-clamp-2 mb-2">
            <a href="{{ $url }}">{{ $title }}</a>
        </h3>
