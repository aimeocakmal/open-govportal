...
OpenGovPortal exposes a REST API under `/api/v1/` for third-party integrations such as mobile apps, kiosks, and other
government portals. The API serves read-only public content (broadcasts, achievements, directory) and accepts contact
form submissions.
...
The API does **not** expose authentication, user management, or any Filament admin operations. The Filament CMS is not
API-driven.
...
Most public endpoints do not require authentication. Future authenticated endpoints (e.g., draft preview) will use
...
API versions are path-prefixed (`/v1/`). Breaking changes will be published under `/v2/` with a deprecation notice
period of 6 months.
...
```json
{
    "error": {
        ...
        "details": {
            ...
            "message": [
                "The message field must be at least 20 characters."
            ]
        }
    }
}
...
**Note:** Staff email and phone are **excluded** from API responses for privacy. Only publicly displayable fields are
returned.
...
**Privacy note:** Fields `email`, `phone`, and `fax` are intentionally omitted from this endpoint. These are displayed
only on the rendered HTML page (`/ms/direktori`), not via the API, to prevent harvesting.
